...
############# Made
    ...
    def background_in_mask(self, crop, mask):    
        """
        Returns mean intensity within mask region of the crop.
        """
        return np.mean(np.ma.masked_equal(crop*mask,0),axis=(0,1,2)).data
    
    def z_masks(self, rz, ref_ch, **kwargs):
        """Using image in ref_ch, returns indexable N x T mask for track array with best z +/- zpad = 1, else 0
            z-offset is an optional argument that is an integer list of form (z_offset_ch0, z_offset_ch1, ...)
        """
        ...
        for n in np.arange(n_tracks):
            for t in np.arange(n_frames):
                ...
                for ch in np.arange(n_channels):
                    ...
                    if best_z[ch] >= self.z_slices()-1:  ## substract 1 since counting starts from zero
                        best_z[ch] = z_slices  ## ensure offset best_z[ch] does no go beyond # of slices
                    elif best_z[ch] < 0:
    ...
    def local_background_subtract(self, masks):
    ...
    def cigar_masks(self, rx, ry, rz, ref_ch, **kwargs):
        """Returns indexable N x T mask from reference channel ref_ch for track array with best z +/- zpad = 1, else 0"""
        z_offset = kwargs.get('z_offset', [0 for i in np.arange(self.n_channels())])
        crop_dim = self.crop_dim()
        n_tracks = self.n_tracks()
        n_frames = self.n_frames()
    ...
    def capsule_masks(self, rx, ry, rz, ref_ch, **kwargs):
        """Returns indexable N x T mask from reference channel ref_ch for track array with best z +/- zpad = 1, else 0"""
        z_offset = kwargs.get('z_offset', [0 for i in np.arange(self.n_channels())])
        crop_dim = self.crop_dim()
        n_tracks = self.n_tracks()
    ...
    def mask_projection(self, array, mask_array): 
        """Performs max-z projection after applying masks to track_array"""
        return np.amax(mask_array*array,0) # multiply arrays and do max_z projection
...
def create_track_array_video(output_directory, output_filename, video_3D, tracks, crop_pad, xy_pixel_size, z_pixel_size):
